...
**After a job succeeds, refresh in place.** On `status: done`, `app.js` issues `htmx.ajax(GET, current_url, {target:"main", select:"main > *", swap:"innerHTML"})`. Never `window.location.reload()` — it kills toast, sidebar focus, and scroll position. `failed` and `cancelled` jobs do NOT trigger a refresh (nothing changed).
